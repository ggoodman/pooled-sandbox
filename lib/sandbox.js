...
Sandbox.prototype.run = function (runConfig, callback) {
    ...
    this.stdout.reset({ output: runConfig.stdout });
    this.stderr.reset({ output: runConfig.stderr });
    
    delete runConfig.stdout;
    delete runConfig.stderr;
...
Sandbox.prototype.onMessage = function(payload) {
    ...
    if (payload.err) {
        ...
        return this.handleError(wrapSandboxError(payload.err));
    }
...
function wrapSandboxError(err) {
    if (err instanceof Error) return err;
    
    var wrappedError;
    
    switch (err.name) {
        case 'SandboxFatalError': wrappedError = new SandboxFatalError(err.message); break;
        case 'SandboxUnhandledError': wrappedError = new SandboxUnhandledError(err.message); break;
        default: wrappedError = new Error(err.message);
    }
    
    Object.defineProperty(err, 'stack', { get: function () { return err.stack; } });
